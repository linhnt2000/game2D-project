...
namespace DarkTonic.MasterAudio {
    ...
    public static class PersistentAudioSettings {
        ...
        public const string VibrateEnabledKey = "MA_vibrateEnabled";
        /*! \endcond */
        ...
        { 
            get
            {
                if (!PlayerPrefs.HasKey(VibrateEnabledKey))
                {
                    return null;
                }

                return PlayerPrefs.GetInt(VibrateEnabledKey) != 0;
            }
            set
            {
                if (!value.HasValue)
                {
                    PlayerPrefs.DeleteKey(VibrateEnabledKey);
                    return;
                }

                var newVal = value.Value;
                PlayerPrefs.SetInt(VibrateEnabledKey, newVal ? 1 : 0);
                //MMVibrationManager.SetHapticsActive(newVal);
            }
        }
